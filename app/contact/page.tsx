...
export default function Contact() {
  ...
  const contactHeroRef = React.useRef(null);
  const contactContentRef = React.useRef(null);
  const mapRef = React.useRef(null);

  const isContactHeroInView = useInView(contactHeroRef, { amount: 0.3, once: false });
  const isContactContentInView = useInView(contactContentRef, { amount: 0.1, once: false });
  ...
  return (
    <div className="flex flex-col w-full min-h-screen pt-24 bg-background font-sans overflow-x-hidden">
      {/* Hero */}
      <section 
        ref={contactHeroRef}
        className="bg-background py-24 border-b border-slate-100 relative overflow-hidden"
      >
        ...
        <div className="container mx-auto px-4 md:px-6 relative z-10">
          <motion.div 
            ...
            initial="hidden"
            animate={isIntroDone && isContactHeroInView ? "visible" : "hidden"}
            className="text-center max-w-3xl mx-auto"
          >
            <motion.h1 variants={itemSlideUp} className="text-4xl md:text-6xl font-black text-slate-900 mb-6 tracking-tight">How Can We <span className="text-pharma-teal">Help?</span></motion.h1>
        ...
        <div className="container mx-auto px-4 md:px-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-20">
            ...
            <motion.div 
              variants={staggerContainer}
              initial="hidden"
              animate={isIntroDone && isContactContentInView ? "visible" : "hidden"}
            ...
            >
              {isSubmitSuccessful ? (
                <div className="py-20 text-center space-y-6">
                  ...
                  <motion.div variants={buttonZoom} initial="hidden" animate="visible">
                    <button 
                      onClick={() => window.location.reload()}
                      className="px-8 py-3 bg-pharma-teal text-white rounded-xl font-bold shadow-lg transition-transform hover:scale-105"
                    >
                      Send Another
                    </button>
                  </motion.div>
      ...
      >
        <div className="container mx-auto px-4 md:px-6">
          <motion.div 
            initial="hidden"
            animate={isIntroDone && isMapInView ? "visible" : "hidden"}
            variants={{
          ...
          >
            ...
            <h2 className="text-4xl font-black text-slate-900 tracking-tight">Regional Distribution Hub</h2>
          </motion.div>
          <motion.div 
            initial="hidden"
            animate={isIntroDone && isMapInView ? "visible" : "hidden"}
               ...
               <p className="text-slate-500 font-bold uppercase tracking-[0.3em] font-subtext">Indore Industrial Hub</p>
                  ...
                  >
                    <motion.a 
                      ...
                      target="_blank" 
                      className="px-10 py-3 bg-white text-pharma-teal rounded-full font-bold shadow-lg flex items-center group-hover:bg-pharma-teal group-hover:text-white transition-all focus:outline-none"
                    >
